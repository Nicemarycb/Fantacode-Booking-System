...
// Create a new show (for testing/setup)
...
router.get('/:showId/availability', async (req, res) => {
  try {
    ...
    // Clean expired holds before calculating availability
    const now = new Date();
...
router.post('/:showId/hold', async (req, res) => {
  ...
  try {
    ...
    const holdDuration = holdDurationMinutes || 5; // Default 5 minutes
...
router.post('/:showId/book', async (req, res) => {
  const session = await Show.startSession();
  session.startTransaction();
...
router.get('/:showId/stats', async (req, res) => {
  try {
    const show = await Show.findById(req.params.showId);
